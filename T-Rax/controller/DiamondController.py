...
class TRaxDiamondController(TRaxModuleController):
    ...
    def create_options_signals(self):
        self.connect_click_function(self.main_view.diamond_control_widget.derivative_show_cb,
                                    self.derivative_show_cb_click)
        self.main_view.diamond_control_widget.derivative_smoothing_sb.valueChanged.connect(
            self.change_derivative_smoothing)
    ...
    def data_changed(self):
        ...
        self.main_view.diamond_control_widget.exp_filename_lbl.setText(
            self.data.exp_data.filename.replace('\\', '/').split('/')[-1])
        self.main_view.diamond_control_widget.exp_folder_name_lbl.setText('/'.
                                                                          join(self.data.exp_data.filename.
                                                                                   replace('\\', '/').split('/')[
                                                                               -3:-1]))
